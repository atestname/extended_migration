...
%   April, 2012: Wrapped into a SPOT operator.
%                Haneet Wason
...
h        = n + XX - YY;
m        = XX + YY - 1;
xs       = (m + h + 1 - n)/2; % When XX = xs, and YY = xr, why calculate xs and xr? 
xr       = (m - h + 1 + n)/2;
h        = h(:);
m        = m(:);
xs       = xs(:);   
...
% Map from source-receiver to midpoint-offest
if nargin < 2
    opinfo = {'opSR2MH', []};
...
fh = @(x,mode) opSR2MH_intrnl(n, x, mode, opinfo, h, m, xs, xr);
op = opFunction(2*n*n, n*n, fh);
