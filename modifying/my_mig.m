...
%image = 1100.*ones(length(z),length(x));
...
for iz = 1:length(z)
    ppi = my_step(data,tf,x,v(iz),(iz)*dz,flag);
    pp = reshape(ppi,length(tf),length(x),length(x));
    % update image
    for ix = 1:length(x)
        image(iz,ix) = image(iz,ix)+pp(1,ix,ix);
    end
%ppp = permute(pp,[2,3,1]);
%figure(2);imagesc(real(pp(:,:,33)));colormap(gray);figure(3);imagesc(abs(image))
%colormap(gray);%colorbar;%caxis([0,3000]);
end
