...
dim = size(data);
x_test = xr; y_test = xs;
K = opDSRmig_image(dim,t,x_test,y_test,z,v);
Kdata = K*vec(data);
image2 = reshape(Kdata,length(z),length(xr));
figure;imagesc(image2); 

% eximage using DSR_exmig
z      = 0:10:1000;
v      = 2000 + 0 * z;
[eximage] = DSR_exmig(data,t,xr,xs,z,v(1:length(z)));


% eximaging with opDSRexmig
dim = size(data);
x_test = xr; y_test = xs;
Kex = opDSRexmig(dim,t,x_test,y_test,z,v);
Kexdata = Kex*vec(data);


% try inverse with DSR_inv
z      = 0:10:1000;
wave = DSR_inv(eximage,t,xr,xs,z,v(1:length(z)));

    % image using Tristan's code
    z      = 0:10:1000;
    v      = 2000 + 0 * z;
    image3 = DSR_mig(wave,t,xr,xs,z,v(1:length(z)));

    figure;imagesc(image3); 
    
    
% try DSR_inv
K = opDSRmig(dim,t,xr,xs,z,v);
dd = K'*vec(eximage);
