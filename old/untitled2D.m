...
% perturbation
epsilon = .1;
dv = 0*xx; dv((zz-50) >= 10) = epsilon*1100;
v = v0+dv;
...
% frequencies [Hz]
model.freq = linspace(-121,121,2^6); nfreq = length(model.freq);
...
model.zsrc = 10;
model.xsrc = 50; nsrc = length(model.xsrc);
model.zrec = 10;
model.xrec = 0:10:150; nrec = length(model.xrec);
